...
void TSPResolver::findOptimalTour(DynamicMemoryTable &table, Graph &graph, vector<int> &result, int previous_vertex) {
    ...
    int temp_vertex;
    //stan koncowy
    int current_state = (1 << graph.getVertixNumber()-1) - 1;
...
TSP_result TSPResolver::resolveUsingBranchAndBound(Graph &graph) {
    ...
    while(!node_queue.empty()) {
        ...
        node_queue.pop();
        if(current_node.getCost() < result.cost) {
            list<Node> children = current_node.getNodeChildren(graph.getVertixNumber());
            for(Node child_node : children) {
                if(child_node.getAmountOfNodesInPath() == graph.getVertixNumber() &&
                        child_node.getCost() < result.cost) {
                    result.cost = child_node.getCost();
                    result.path = child_node.getPath();
                }
                else if(child_node.getCost() < result.cost)
